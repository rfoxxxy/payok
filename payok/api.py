...
from .enums import ComissionType, Currency, PaymentMethod, PayoutMethod
from .exceptions import PayOKError
from .models import Balance, NewPayout, Payout, Transaction
...
class PayOK(object):
    ...
    def __init__(
        ...
        :param Optional[httpx.AsyncClient] session: HttpX client session
        """
    ...
    def _check_response(response: dict) -> None:
        ...
        if response.get("status") == "error":
            raise PayOKError(
                response["error_code"],
                response.get("error_text") or response.get("text"),  # type: ignore
            )
    ...
    async def get_transactions(
        ...
        result.pop("status")
        return [Transaction(**transaction) for transaction in result.values()]

    async def get_payouts(
        self, payout_id: Optional[int] = None, offset: Optional[int] = None
    ) -> List[Payout]:
        """
        ...
        result.pop("status")
        return [Payout(**payout) for payout in result.values()]
